...
export default function Sidebar({ isOpen, toggleMenu }: { isOpen: boolean, toggleMenu: () => void }) {
  return (<>
    <div className={
      ...
      <header className="w-full text-white px-5 md:px-10 py-3 lg:hidden">
      ...
      <div className="text-xl grid content-center overflow-y-auto lg:max-w-(--sidebar-size)">
        {
          routes.map((route) => (
            <Link
              key={route.path}
      ...
      <Link
        className="min-w-screen w-screen text-xl px-5 py-3 flex items-center gap-2 transition-all duration-200 hover:text-white hover:bg-(--background-blue-hover) focus:bg-(--background-blue-hover)"
        to="/login"
      >
